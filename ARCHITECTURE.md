...
## 7. ملاحظات تقنية وقرارات هامة

### القرار: طريقة تحميل بيانات JSON المحلية في بيئة Vite

- **المشكلة التي تمت مواجهتها**: فشل تحميل بيانات الأنماط الفنية من ملف `local_image_prompt_components.json`، مما أدى إلى ظهور قائمة منسدلة فارغة في واجهة المستخدم.

- **التحليل والسبب الجذري**: كان الكود يستخدم دالة `fetch()` لمحاولة تحميل ملف `JSON` من مسار مثل `/data/filename.json`. هذا النهج يفشل في بيئة Vite لأن خادم التطوير لا يقوم بتقديم الملفات من خارج مجلد `public` بشكل افتراضي.

- **الحل المعتمد (النهج الصحيح)**: تم تغيير طريقة تحميل البيانات بشكل جذري. بدلاً من محاولة تحميل الملف في وقت التشغيل (Runtime)، تم التحول إلى **الاستيراد المباشر** للملف في وقت البناء (Build-time).

  ```typescript
  // الطريقة الخاطئة (لا تعمل بشكل موثوق في Vite)
  // fetch('/data/local_image_prompt_components.json').then(...)

  // الطريقة الصحيحة (موصى بها في Vite)
  import imageData from '../../data/local_image_prompt_components.json';
  ```

- **النتيجة**: هذا يضمن أن Vite يقوم بتضمين ملف `JSON` مباشرة في حزمة JavaScript النهائية. هذا الحل أكثر موثوقية، وأفضل من ناحية الأداء، ويمنع حدوث أخطاء الشبكة غير المتوقعة لتحميل الأصول المحلية الأساسية.
